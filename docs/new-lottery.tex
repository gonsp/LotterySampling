...
\item For top-$k$ queries, $R_i$ will denote
  ...
  $\text{rank}(x)$ is efined as 1 plus the number of elements in the stream
  with frequency strictly larger than $x$. Thus, if all frequencies
  were distinct, $\text{rank}(x_i)=i$, for all $i$, $1\le i\le n$.
  We also introduce
  $\rho_i$, the actual relative frequency of the element reported as $i$-th
  most frequent by the algorithm. Notice that $\rho_i = p_{R_i}$.
\item For any element $x_i$, we denote $f'_i$ the estimated absolute frequency
  of $x_i$ (by convention, w set $f'_i=0$ if $x_i$ is not in the sample).
